...
def determinarMejorJugada(tablero):
    # Verificar primero si hay una jugada ganadora inmediata
    for fila in range(3):
        for columna in range(3):
            if tablero.getterSimbolo(fila, columna) == "":
                # Probar este movimiento
                tablero.setterSimbolo(fila, columna, "O")

                # Si es ganador, devolver esta jugada inmediatamente
                if finalizarJuego(tablero, "O"):
                    # Deshacer la prueba
                    tablero.setterSimbolo(fila, columna, "")
                    return (fila, columna)

                # Deshacer la prueba
                tablero.setterSimbolo(fila, columna, "")

    # Verificar si necesitamos bloquear una jugada ganadora del oponente
    for fila in range(3):
        for columna in range(3):
            if tablero.getterSimbolo(fila, columna) == "":
                # Probar este movimiento como si fuera del oponente
                tablero.setterSimbolo(fila, columna, "X")

                # Si el oponente ganaría, bloquearlo
                if finalizarJuego(tablero, "X"):
                    # Deshacer la prueba
                    tablero.setterSimbolo(fila, columna, "")
                    return (fila, columna)
    ...
    # Si no hay jugada inmediata, crear el árbol y aplicar MIN-MAX
...
def jugarTriqui():
    ...
    while True:
        ...
        if turno == "X":  # Turno del jugador humano
            print(f"Tu turno (X)")
            try:
